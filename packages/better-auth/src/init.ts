...
import { ButteryAuthError } from "./error";

export const init = async (options: ButteryAuthOptions) => {
	...
	const ctx: AuthContext = {
		appName: options.appName || "Buttery Auth",
...
export type AuthContext = {
	...
	authCookies: ButteryAuthCookies;
...
function runPluginInit(ctx: AuthContext) {
	...
	const dbHooks: ButteryAuthOptions["databaseHooks"][] = [];
...
function getInternalPlugins(options: ButteryAuthOptions) {
	const plugins: ButteryAuthPlugin[] = [];
...
function getTrustedOrigins(options: ButteryAuthOptions) {
	const baseURL = getBaseURL(options.baseURL, options.basePath);
	if (!baseURL) {
		return [];
	}

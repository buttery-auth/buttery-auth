...
import type { Adapter, ButteryAuthOptions, Where } from "../../types";
...
export const createAdapter =
	...
	(options: ButteryAuthOptions): Adapter => {
		const config = {
			...cfg,
		...
		// End-user's Buttery-Auth instance's schema
		...
		const getDefaultFieldName = ({
			...
			// Plugin `schema`s can't define their own `id`. Buttery-auth auto provides `id` to every schema model.
					...
					? {
							defaultValue() {
								...
								if (options.advanced?.generateId !== undefined) {
									logger.warn(
										"Your Buttery Auth config includes advanced.generateId which is deprecated. Please use advanced.database.generateId instead. This will be removed in future releases.",
									);

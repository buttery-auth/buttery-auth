...
export async function handleOAuthUserInfo(
	...
	const dbUser = await c.context.internalAdapter
		...
		.catch((e) => {
			logger.error(
				"Buttery auth was unable to query your database.\nError: ",
				e,
			);
			const errorURL =
	...
	if (dbUser) {
		...
		if (!hasBeenLinked) {
			...
			if (
				...
				if (isDevelopment) {
					logger.warn(
						`User already exist but account isn't linked to ${account.providerId}. To read more about how account linking works in Buttery Auth see https://www.buttery-auth.com/docs/concepts/users-accounts#account-linking.`,
					);
				}
				return {

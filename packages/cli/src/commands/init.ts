...
/**
 * Should only use any database that is core DBs, and supports the Buttery Auth CLI generate functionality.
 */
...
export async function initAction(opts: any) {
	console.log();
	intro("👋 Initializing Buttery Auth");
	...
	if (
		...
		log.warn(
			`Buttery Auth requires your tsconfig.json to have "compilerOptions.strict" set to true.`,
		);
	...
	if (
		...
		const shouldInstallButteryAuthDep = await confirm({
			message: `Would you like to install Buttery Auth?`,
		});
		if (isCancel(shouldInstallButteryAuthDep)) {
			cancel(`✋ Operation cancelled.`);
			process.exit(0);
		}
		...
		if (shouldInstallButteryAuthDep) {
			s2.start(
				`Installing Buttery Auth using ${chalk.bold(packageManagerPreference)}`,
			);
			try {
				...
				s2.stop(
					`Buttery Auth installed ${chalk.greenBright(
						`successfully`,
					)}! ${chalk.gray(`(${formatMilliseconds(Date.now() - start)})`)}`,
				);
			} catch (error: any) {
				s2.stop(`Failed to install Buttery Auth:`);
				console.error(error);
				process.exit(1);
			}
		}
		...
		s.stop("Finished fetching latest version of better-auth.");
		const shouldInstallButteryAuthDep = await confirm({
			message: `Your current Buttery Auth dependency is out-of-date. Would you like to update it? (${chalk.bold(
		...
		if (shouldInstallButteryAuthDep) {
			...
			const s = spinner({ indicator: "dots" });
			s.start(
				`Updating Buttery Auth using ${chalk.bold(packageManagerPreference)}`,
			);
			try {
				const start = Date.now();
				await installDependencies({
				...
				s.stop(
					`Buttery Auth updated ${chalk.greenBright(
						`successfully`,
					)}! ${chalk.gray(`(${formatMilliseconds(Date.now() - start)})`)}`,
				);
			} catch (error: any) {
				s.stop(`Failed to update Buttery Auth:`);
				log.error(error.message);
				process.exit(1);
			}
		}
	} else {
		s.stop(`Buttery Auth dependencies are ${chalk.greenBright(`up to date`)}!`);
	}

...
# Buttery Auth 1.2 – Stripe, Captcha, API Keys, Teams, Init CLI, and more

To upgrade, run:

```package-install
npm install buttery-auth@1.2
```
...
```package-install
npm install @buttery-auth/stripe
```

```ts title="auth.ts"
import { stripe } from "@buttery-auth/stripe"; // [!code highlight]
...
```ts title="auth.ts"
import { captcha } from "buttery-auth/plugins";
...
```ts title="auth.ts"
import { apiKey } from "buttery-auth/plugins";
...
The CLI now includes an `init` command to add Buttery Auth to your project.

```bash title="terminal"
npx @buttery-auth/cli init
```
...
We rewrote **buttery-call** (the core library behind Buttery Auth) to fix TypeScript editor lag. Your IDE should now feel much snappier when working with Buttery Auth.
...
A lot of fixes and refinements to make everything smoother, faster, and more reliable. Check out the [changelog](https://github.com/buttery-auth/buttery-auth/releases/tag/v1.2.0) for more details.

---

```package-install
npm install buttery-auth@1.2.0
```

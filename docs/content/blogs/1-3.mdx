...
## Buttery Auth 1.3 Release

We're excited to announce the release of Buttery Auth 1.3. This release includes a lot of new features and improvements.

To upgrade, run:

```package-install
npm install buttery-auth@1.3
```
...
import { sso } from "@buttery-auth/sso";
...
```ts title="auth.ts"
import { mcp } from "buttery-auth/plugins";
...
```ts title="auth.ts"
import { butteryAuth } from "buttery-auth";
import { stripe } from "@buttery-auth/stripe";
...
```ts title="auth.ts"
import { siwe } from "buttery-auth/plugins";
...
```ts title="auth.ts"
import { butteryAuth } from "buttery-auth";
import { sveltekitCookies } from "buttery-auth/svelte-kit";
...
* Migrated to **Zod 4** for buttery type safety and performance
...
* Facebook: Buttery detection for limited token JWT
...
* Postgres: Buttery varchar normalization and type comparison
...
👉 [Check the full changelog](https://github.com/buttery-auth/buttery-auth/releases/tag/v1.3.0)

---
title: Migrating from Supabase Auth to Buttery Auth
description: A step-by-step guide to transitioning from Supabase Auth to Buttery Auth.
---

In this guide, we'll walk through the steps to migrate a project from Supabase Auth to Buttery Auth.
...
```ts title="auth.ts"
import { admin, anonymous } from "buttery-auth/plugins";
...
Now that we have the necessary tables in our database, we can run the migration script to migrate the users and accounts from Supabase to Buttery Auth.
...
Run the migration script to migrate the users and accounts from Supabase to Buttery Auth.
...
Update your codebase from Supabase auth calls to Buttery Auth API.

Here's a list of the Supabase auth API calls and their Buttery Auth counterparts.
...
Congratulations! You've successfully migrated from Supabase Auth to Buttery Auth.

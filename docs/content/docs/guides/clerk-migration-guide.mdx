---
title: Migrating from Clerk to Buttery Auth
description: A step-by-step guide to transitioning from Clerk to Buttery Auth.
---

In this guide, we'll walk through the steps to migrate a project from Clerk to Buttery Auth — including email/password with proper hashing, social/external accounts, phone number, two-factor data, and more.
...
Before starting the migration process, set up Buttery Auth in your project. Follow the [installation guide](/docs/installation) to get started. And go to
...
import { Pool } from "pg";
import { butteryAuth } from "buttery-auth";
import { admin, twoFactor, phoneNumber, username } from "buttery-auth/plugins";
...
import { generateRandomString, symmetricEncrypt } from "buttery-auth/crypto";
...
<Step>
  ...
  3. Verify the migrated data in Buttery Auth before proceeding
  ...
  Now that the data is migrated, you can start updating your components to use Buttery Auth. Here's an example for the sign-in component:

  ```tsx title="components/auth/sign-in.tsx"
  import { authClient } from "buttery-auth/client";
...
<Step>
  ...
  Replace your Clerk middleware with Buttery Auth's middleware:

  ```ts title="middleware.ts"

  import { NextRequest, NextResponse } from "next/server";
  import { getSessionCookie } from "buttery-auth/cookies";
  ...
  Once you've verified that everything is working correctly with Buttery Auth, you can remove Clerk:
...
[Goodbye Clerk, Hello Buttery Auth – Full Migration Guide!](https://www.youtube.com/watch?v=Za_QihbDSuk)

## Wrapping Up

Congratulations! You've successfully migrated from Clerk to Buttery Auth.

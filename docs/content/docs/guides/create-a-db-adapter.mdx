...
description: Learn how to create a custom database adapter for Buttery-Auth
---

Learn how to create a custom database adapter for Buttery-Auth using `createAdapter`.

Our `createAdapter` function is designed to be very flexible, and we've done our best to make it easy to understand and use.
Our hope is to allow you to focus on writing database logic, and not have to worry about how the adapter is working with Buttery-Auth.
...
```ts
import { createAdapter, type AdapterDebugLogs } from "buttery-auth/adapters";
...
The `config` object is mostly used to provide information about the adapter to Buttery-Auth.
...
- `options`: The Buttery Auth options.
- `schema`: The schema from the user's Buttery Auth instance.
...
<Callout>
Note:
If the user has enabled the `useNumberId` option, or if `generateId` is `false` in the user's Buttery Auth config,
...
The `createSchema` method allows the [Buttery Auth CLI](/docs/concepts/cli) to [generate](/docs/concepts/cli/#generate) a schema for the database.

parameters:

- `tables`: The tables from the user's Buttery-Auth instance schema; which is expected to be generated into the schema file.
...
import { runAdapterTest } from "buttery-auth/adapters/test";
...
describe("My Adapter Tests", async () => {
  ...
  await runAdapterTest({
    getAdapter: async (butteryAuthOptions = {}) => {
      return adapter(butteryAuthOptions);
    },
  });
});
```
...
import { expect, test, describe } from "vitest";
import { runNumberIdAdapterTest } from "buttery-auth/adapters/test";
import { myAdapter } from "./my-adapter";
...
The `config` object is used to provide information about the adapter to Buttery-Auth.
...
For example, MongoDB uses `_id` while in Buttery-Auth we use `id`.
...
mapKeysTransformOutput: () => {
  return {
    _id: "id", // We want to replace `_id` (from MongoDB) to `id` (for Buttery-Auth)
  };
},
```

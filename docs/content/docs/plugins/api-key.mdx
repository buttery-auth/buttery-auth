---
title: API Key
description: API Key plugin for Buttery Auth.
---
...
<Steps>
    ...
    <Step>
        ...
        import { apiKeyClient } from "buttery-auth/client/plugins"
...
Any time an endpoint in Buttery Auth is called that has a valid API key in the headers, we will automatically create a mock session to represent the user.
...
For every other endpoint/method, you should utilize Buttery Auth's [built-in rate-limiting](/docs/concepts/rate-limit).

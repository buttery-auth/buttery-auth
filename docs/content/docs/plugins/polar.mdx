---
title: Polar
description: Buttery Auth Plugin for Payment and Checkouts using Polar
---

[Polar](https://polar.sh) is a developer first payment infrastructure. Out of the box it provides a lot of developer first integrations for payments, checkouts and more. This plugin helps you integrate Polar with Buttery Auth to make your auth + payments flow seamless.
...
```bash
pnpm add buttery-auth @polar-sh/buttery-auth @polar-sh/sdk
```
...
### Configuring ButteryAuth Server
...
import { polar, checkout, portal, usage, webhooks } from "@polar-sh/buttery-auth";
...
### Configuring ButteryAuth Client

You will be using the ButteryAuth Client to interact with the Polar functionalities.

```typescript
import { createAuthClient } from "buttery-auth/react";
import { polarClient } from "@polar-sh/buttery-auth";
import { organizationClient } from "buttery-auth/client/plugins";

// This is all that is needed
// All Polar plugins, etc. should be attached to the server-side ButteryAuth config
...
When `createCustomerOnSignUp` is enabled, a new Polar Customer is automatically created when a new User is added in the Buttery-Auth Database.
...
```typescript
import { polar, checkout } from "@polar-sh/buttery-auth";
...
When checkouts are enabled, you're able to initialize Checkout Sessions using the checkout-method on the ButteryAuth Client. This will redirect the user to the Product Checkout.
...
```typescript
import { polar, checkout, portal } from "@polar-sh/buttery-auth";
...
The portal-plugin gives the ButteryAuth Client a set of customer management methods, scoped under `authClient.customer`.
...
```typescript
import { polar, checkout, portal, usage } from "@polar-sh/buttery-auth";
...
```typescript
import { polar, webhooks } from "@polar-sh/buttery-auth";

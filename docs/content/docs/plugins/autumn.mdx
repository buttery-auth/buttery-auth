...
description: Buttery Auth Plugin for Autumn Billing
---
...
<Steps>
  ...
  <Step>
    ...
    ```ts title="auth.ts"
    import { autumn } from "autumn-js/buttery-auth";
  ...
  <Step>
      ...
      Client side, wrap your application with the AutumnProvider component, and pass in the `baseUrl` that you define within buttery-auth's `authClient`.

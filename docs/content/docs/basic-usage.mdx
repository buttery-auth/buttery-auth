---
title: Basic Usage
description: Getting started with Buttery Auth
---

Buttery Auth provides built-in authentication support for:
...
```ts title="server.ts"
import { auth } from "./auth"; // path to your Buttery Auth server instance
...
Buttery Auth supports multiple social providers, including Google, GitHub, Apple, Discord, and more. To use a social provider, you need to configure the ones you need in the `socialProviders` option on your `auth` object.
...
Once a user is signed in, you'll want to access the user session. Buttery Auth allows you easily to access the session data from the server and client side.

### Client Side

#### Use Session

Buttery Auth provides a `useSession` hook to easily access session data on the client side. This hook is implemented using nanostore and has support for each supported framework and vanilla client, ensuring that any changes to the session (such as signing out) are immediately reflected in your UI.
...
<Tabs items={["Next.js", "Nuxt", "Svelte", "Astro", "Hono", "TanStack"]}>
    ...
    <Tab value="Remix">
    ```ts title="route.ts"
    import { auth } from "lib/auth"; // path to your Buttery Auth server instance
...
One of the unique features of Buttery Auth is a plugins ecosystem. It allows you to add complex auth related functionality with small lines of code.
...
```ts title="auth.ts"
import { butteryAuth } from "buttery-auth"
import { twoFactor } from "buttery-auth/plugins" // [!code highlight]
...
import { twoFactorClient } from "buttery-auth/client/plugins"; // [!code highlight]

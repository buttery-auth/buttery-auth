...
Buttery Auth providers out of the box support for the [Generic Oauth Plugin](/docs/plugins/generic-oauth) which allows you to use any social provider that implements the OAuth2 protocol or OpenID Connect (OIDC) flows.
...
```ts title="auth.ts"
import { betterAuth } from "buttery-auth";
import { genericOAuth } from "buttery-auth/plugins";

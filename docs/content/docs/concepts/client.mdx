---
title: Client
description: Buttery Auth client library for authentication.
---

Buttery Auth offers a client library compatible with popular frontend frameworks like React, Vue, Svelte, and more. This client library includes a set of functions for interacting with the Buttery Auth server. Each framework's client library is built on top of a core client library that is framework-agnostic, so that all methods and hooks are consistently available across all client libraries.

## Installation

If you haven't already, install buttery-auth.

```package-install
npm i @buttery-auth/buttery-auth
```

## Create Client Instance

Import `createAuthClient` from the package for your framework (e.g., "buttery-auth/react" for React). Call the function to create your client. Pass the base URL of your auth server. If the auth server is running on the same domain as your client, you can skip this step.
...
Once you've created your client instance, you can use the client to interact with the Buttery Auth server. The client provides a set of functions by default and they can be extended with plugins.
...
The client uses a library called [buttery fetch](https://better-fetch.vercel.app) to make requests to the server.

Buttery fetch is a wrapper around the native fetch API that provides a more convenient way to make requests. It's created by the same team behind Buttery Auth and is designed to work seamlessly with it.
...
```ts title="auth-client.ts"
import { createAuthClient } from "buttery-auth/client"
import { magicLinkClient } from "buttery-auth/client/plugins"

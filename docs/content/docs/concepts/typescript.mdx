---
title: TypeScript
description: Buttery Auth TypeScript integration.
---

Buttery Auth is designed to be type-safe. Both the client and server are built with TypeScript, allowing you to easily infer types.
...
Buttery Auth is designed to work with TypeScript's strict mode. We recommend enabling strict mode in your TypeScript config file:
...
Buttery Auth allows you to add additional fields to the user and session objects. All additional fields are properly inferred and available on the server and client side.
...
```ts
import { inferAdditionalFields } from "buttery-auth/client/plugins";

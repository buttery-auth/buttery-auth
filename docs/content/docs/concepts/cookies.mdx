---
title: Cookies
description: Learn how cookies are used in Buttery Auth.
---
...
Buttery Auth cookies will follow `${prefix}.${cookie_name}` format by default. The prefix will be "buttery-auth" by default. You can change the prefix by setting `cookiePrefix` in the `advanced` object of the auth options.
...
By default, Buttery Auth uses the following cookies:

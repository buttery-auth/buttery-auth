---
title: Plugins
description: Learn how to use plugins with Buttery Auth.
---

Plugins are a key part of Buttery Auth, they let you extend the base functionalities. You can use them to add new authentication methods, features, or customize behaviors.

Buttery Auth comes with many built-in plugins ready to use. Check the plugins section for details. You can also create your own plugins.
...
The Buttery Auth auth client on the frontend uses the `createAuthClient` function provided by `buttery-auth/client`.

```ts title="auth-client.ts"
import { createAuthClient } from "buttery-auth/client";
...
```ts title="plugin.ts"
import type { BetterAuthPlugin } from "buttery-auth";
...
To create an Auth Endpoint you'll need to import `createAuthEndpoint` from `buttery-auth`.

Buttery Auth uses wraps around another library called <Link href="https://github.com/bekacru/better-call"> Better Call </Link> to create endpoints. Better call is a simple ts web framework made by the same team behind Buttery Auth.

```ts title="plugin.ts"
import { createAuthEndpoint } from "buttery-auth/api";
...
Create Auth endpoints wraps around `createEndpoint` from Better Call. Inside the `ctx` object, it'll provide another object called `context` that give you access buttery-auth specific contexts including `options`, `db`, `baseURL` and more.

**Context Object**

- `appName`: The name of the application. Defaults to "Buttery Auth".
- `options`: The options passed to the Buttery Auth instance.
...
- `logger`: The logger instance used by Buttery Auth.
- `db`: The Kysely instance used by Buttery Auth to interact with the database.
...
- `internalAdapter`: These are internal db calls that are used by Buttery Auth. For example, you can use these calls to create a session instead of using `adapter` directly. `internalAdapter.createSession(userId)`
...
For other properties, you can check the <Link href="https://github.com/bekacru/better-call">Better Call</Link> documentation and the <Link href="https://github.com/buttery-auth/buttery-auth/blob/main/packages/buttery-auth/src/init.ts">source code </Link>.
...
```ts title="plugin.ts"
import { BetterAuthPlugin } from "buttery-auth/plugins";
...
By default buttery-auth will create an `id` field for each table. You can add additional fields to the table by adding them to the `fields` object.
...
```ts title="plugin.ts"
import {  createAuthMiddleware } from "buttery-auth/plugins";
...
```ts title="plugin.ts"
import { createAuthMiddleware } from "buttery-auth/plugins";
import { sessionMiddleware } from "buttery-auth/api";
...
If your endpoints needs to be called from the client, you'll need to also create a client plugin. Buttery Auth clients can infer the endpoints from the server plugins. You can also add additional client side logic.

```ts title="client-plugin.ts"
import type { BetterAuthClientPlugin } from "buttery-auth";
...
```ts title="client-plugin.ts"
import type { BetterAuthClientPlugin } from "buttery-auth/client";
...
Buttery Auth uses <Link href="https://better-fetch.vercel.app"> Better fetch </Link> to make requests. Better fetch is a simple fetch wrapper made by the same author of Buttery Auth.

...
description: Buttery Auth session management.
---

Buttery Auth manages session using a traditional cookie-based session management. The session is stored in a cookie and is sent to the server on every request. The server then verifies the session and returns the user data if the session is valid.
...
Some endpoints in Buttery Auth require the session to be **fresh**. A session is considered fresh if its `createdAt` is within the `freshAge` limit. By default, the `freshAge` is set to **1 day** (60 * 60 * 24).
...
Buttery Auth provides a set of functions to manage sessions.
...
```ts title="auth.ts"
import { customSession } from "buttery-auth/plugins";
...
```ts title="auth-client.ts"
import { customSessionClient } from "buttery-auth/client/plugins";
...
```ts
import { betterAuth, ButteryAuthOptions } from "buttery-auth";

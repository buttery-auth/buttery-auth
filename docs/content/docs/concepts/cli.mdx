...
Buttery Auth comes with a built-in CLI to help you manage the database schemas, initialize your project, and generate a secret key for your application.
...
- `--config` - The path to your Buttery Auth config file. By default, the CLI will search for a auth.ts file in **./**, **./utils**, **./lib**, or any of these directories under `src` directory.
...
The `init` command allows you to initialize Buttery Auth in your project.
...
If you see this error, it means the CLI can’t resolve imported modules in your Buttery Auth config file. We're working on a fix for many of these issues, but in the meantime, you can try the following:

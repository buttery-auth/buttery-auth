...
description: Integrate Buttery Auth with Next.js.
---

Buttery Auth can be easily integrated with Next.js. Before you start, make sure you have a Buttery Auth instance configured. If you haven't done that yet, check out the [installation](/docs/installation).
...
import { createAuthClient } from "buttery-auth/react" // make sure to import from buttery-auth/react
...
import { nextCookies } from "buttery-auth/next-js";
...
You can use the `getSessionCookie` helper from Buttery Auth for this purpose:
...
```ts
import { getCookieCache } from "buttery-auth/cookies";

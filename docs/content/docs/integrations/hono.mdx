...
description: Integrate Buttery Auth with Hono.
---
...
By default, all Buttery Auth cookies are set with `SameSite=Lax`. If you need to use cookies across different domains, you’ll need to set `SameSite=None` and `Secure=true`. However, we recommend using subdomains whenever possible, as this allows you to keep `SameSite=Lax`. To enable cross-subdomain cookies, simply turn on `crossSubDomainCookies` in your auth config.
...
When using the Hono client (`@hono/client`) to make requests to your Buttery Auth-protected endpoints, you need to configure it to send credentials (cookies) with cross-origin requests.
							...
							<span className="font-medium flex gap-2 text-sm text-zinc-700 dark:text-zinc-300">
								<span className=" text-zinc-900 dark:text-white/90 hover:text-zinc-950 text-xs md:text-sm dark:hover:text-zinc-100 transition-colors">
									...
									<span className="font-semibold">
										Buttery Auth Infrastructure
									</span>
								</span>
								<span className=" text-zinc-400 hidden md:block">|</span>
								<Link
									href="https://buttery-auth.build"

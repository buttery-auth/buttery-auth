...
description: Integrate Buttery Auth with Expo.
---

Expo is a popular framework for building cross-platform apps with React Native. Buttery Auth supports both Expo native and web apps.

## Installation

<Steps>
    <Step>
        ## Configure A Buttery Auth Backend
        Before using Buttery Auth with Expo, make sure you have a Buttery Auth backend set up. You can either use a separate server or leverage Expo's new [API Routes](https://docs.expo.dev/router/reference/api-routes) feature to host your Buttery Auth instance.

        To get started, check out our [installation](/docs/installation) guide for setting up Buttery Auth on your server. If you prefer to check out the full example, you can find it [here](https://github.com/buttery-auth/buttery-auth/tree/main/examples/expo-example).

        To use the new API routes feature in Expo to host your Buttery Auth instance you can create a new API route in your Expo app and mount the Buttery Auth handler.

        ```ts title="app/api/auth/[...auth]+api.ts"
        import { auth } from "@/lib/auth"; // import Buttery Auth handler
    ...
    <Step>
        ...
        Install both the Buttery Auth package and Expo plugin into your server application.

        ```package-install
        buttery-auth @buttery-auth/expo
        ```
    ...
    <Step>
        ...
        You also need to install both the Buttery Auth package and Expo plugin into your Expo application.
    ...
    <Step>
        ...
        Add the Expo plugin to your Buttery Auth server.

        ```ts title="lib/auth.ts"
        import { betterAuth } from "buttery-auth";
        import { expo } from "@buttery-auth/expo";
    ...
    <Step>
        ## Initialize Buttery Auth Client
        
        To initialize Buttery Auth in your Expo app, you need to call `createAuthClient` with the base URL of your Buttery Auth backend. Make sure to import the client from `/react`.
        ...
        You need to also import client plugin from `@buttery-auth/expo/client` and pass it to the `plugins` array when initializing the auth client.
        ...
        import { expoClient } from "@buttery-auth/expo/client";
        import * as SecureStore from "expo-secure-store";

        export const authClient = createAuthClient({
            baseURL: "http://localhost:8081", // Base URL of your Buttery Auth backend.
            plugins: [
                expoClient({
                    scheme: "myapp",
                    storagePrefix: "myapp",
     ...
     <Step>
        ...
        Buttery Auth uses deep links to redirect users back to your app after authentication. To enable this, you need to add your app's scheme to the `trustedOrigins` list in your Buttery Auth config. 
        ...
        Then, update your Buttery Auth config to include the scheme in the `trustedOrigins` list.
    ...
    <Step>
        ...
        To resolve Buttery Auth exports you'll need to enable `unstable_enablePackageExports` in your metro config. 
            ...
            return {
                ...
                plugins: [
                    [
                        ...
                        {
                            alias: {
                                "buttery-auth/react": "./node_modules/buttery-auth/dist/client/react/index.cjs",
                                "buttery-auth/client/plugins": "./node_modules/buttery-auth/dist/client/plugins/index.cjs",
                                "@buttery-auth/expo/client": "./node_modules/@buttery-auth/expo/dist/client.cjs",
                            },
                        },
                    ],
                ],
...
With Buttery Auth initialized, you can now use the `authClient` to authenticate users in your Expo app.
...
Buttery Auth provides a `useSession` hook to access the current user's session in your app.
...
**scheme**: scheme is used to deep link back to your app after a user has authenticated using oAuth providers. By default, Buttery Auth tries to read the scheme from the `app.json` file. If you need to override this, you can pass the scheme option to the client.

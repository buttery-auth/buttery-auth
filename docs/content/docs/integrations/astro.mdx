...
description: Integrate Buttery Auth with Astro.
---

Buttery Auth comes with first class support for Astro. This guide will show you how to integrate Buttery Auth with Astro.
...
To enable Buttery Auth to handle requests, we need to mount the handler to a catch all API route. Create a file inside `/pages/api/auth` called `[...all].ts` and add the following code:
...
declare namespace App {
    ...
    interface Locals {
        user: import("buttery-auth").User | null;
        session: import("buttery-auth").Session | null;
    }
}
```

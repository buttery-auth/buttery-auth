...
description: Integrate Buttery Auth with Nuxt.
---
...
```
<Callout type="info">
 You can change the path on your buttery-auth configuration but it's recommended to keep it as `/api/auth/[...all]`
</Callout>
...
import { createAuthClient } from "buttery-auth/vue" // make sure to import from buttery-auth/vue
...
The `api` object exported from the auth instance contains all the actions that you can perform on the server. Every endpoint made inside Buttery Auth is a invocable as a function. Including plugins endpoints.
...
- [Nuxt and Nuxt Hub example](https://github.com/atinux/nuxthub-buttery-auth) on GitHub.
- [NuxtZzle is Nuxt,Drizzle ORM example](https://github.com/leamsigc/nuxt-buttery-auth-drizzle) on GitHub [preview](https://nuxt-buttery-auth.giessen.dev/)
- [Nuxt example](https://stackblitz.com/github/buttery-auth/buttery-auth/tree/main/examples/nuxt-example) on StackBlitz.
- [NuxSaaS (Github)](https://github.com/NuxSaaS/NuxSaaS) is a full-stack SaaS Starter Kit that leverages Buttery Auth for secure and efficient user authentication. [Demo](https://nuxsaas.com/)

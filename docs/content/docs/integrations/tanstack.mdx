...
description: Integrate Buttery Auth with TanStack Start.
---
...
- When you call functions that need to set cookies (like `signInEmail` or `signUpEmail`), you'll need to handle cookie setting for TanStack Start. Buttery Auth provides a `reactStartCookies` plugin to automatically handle this for you.

```ts title="src/lib/auth.ts"
import { betterAuth } from "buttery-auth";
import { reactStartCookies } from "buttery-auth/react-start";

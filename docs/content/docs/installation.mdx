---
title: Installation
description: Learn how to configure Buttery Auth in your project.
---
...
Let's start by adding Buttery Auth to your project:
...
<Callout type="info">
If you're using a separate client and server setup, make sure to install Buttery Auth in both parts of your project.
</Callout>
</Step>
...
<Step>
### Create A Buttery Auth Instance
...
And in this file, import Buttery Auth and create your auth instance. Make sure to export the auth instance with the variable name `auth` or as a `default` export.
...
Buttery Auth requires a database to store user data.
You can easily configure Buttery Auth to use SQLite, PostgreSQL, or MySQL, and more!

<Tabs items={["sqlite", "postgres", "mysql"]}>
    <Tab value="sqlite">
    ```ts title="auth.ts"
    import { butteryAuth } from "buttery-auth";
    import Database from "buttery-sqlite3";
...
<Tabs items={["drizzle", "prisma", "mongodb"]}>
    ...
    ```ts title="auth.ts"
    import { butteryAuth } from "buttery-auth";
    import { drizzleAdapter } from "buttery-auth/adapters/drizzle";
    ...
    <Tab value="prisma">
        ```ts title="auth.ts"
        import { butteryAuth } from "buttery-auth";
        import { prismaAdapter } from "buttery-auth/adapters/prisma";
    ...
    ```ts title="auth.ts"
    import { butteryAuth } from "buttery-auth";
    import { mongodbAdapter } from "buttery-auth/adapters/mongodb";
...
<Step>
    ...
    Buttery Auth includes a CLI tool to help manage the schema required by the library.
...
Configure the authentication methods you want to use. Buttery Auth comes with built-in support for email/password, and social sign-on providers.
...
<Callout>
  Buttery Auth supports any backend framework with standard Request and Response
...
<Tabs items={["next-js", "nuxt", "svelte-kit", "remix", "solid-start", "hono", "express", "elysia", "tanstack-start", "expo"]} defaultValue="react">
    <Tab value="next-js">
        ...
        import { toNextJsHandler } from "buttery-auth/next-js";
    ...
    import { auth } from "$lib/auth"; // path to your auth file
    import { svelteKitHandler } from "buttery-auth/svelte-kit";
    ...
    import { auth } from "~/lib/auth"; // path to your auth file
    import { toSolidStartHandler } from "buttery-auth/solid-start";
    ...
    app.listen(port, () => {
        console.log(`Buttery Auth app listening on port ${port}`);
    });
    ```
    ...
    const butteryAuthView = (context: Context) => {
    ...
    const app = new Elysia().all("/api/auth/*", butteryAuthView).listen(3000);
...
The client-side library helps you interact with the auth server. Buttery Auth comes with a client for all the popular web frameworks, including vanilla JavaScript.

1. Import `createAuthClient` from the package for your framework (e.g., "buttery-auth/react" for React).
...
<Tabs items={["react", "vue", "svelte", "solid",
  "vanilla"]} defaultValue="react">
    ...
    <Tab value="vue" title="lib/auth-client.ts">
            ```ts  title="lib/auth-client.ts"
            import { createAuthClient } from "buttery-auth/vue"
            export const authClient = createAuthClient({
                /** The base URL of the server (optional if you're using the same domain) */ // [!code highlight]
                baseURL: "http://localhost:3000" // [!code highlight]
            })
    ...
    <Tab value="svelte" title="lib/auth-client.ts">
            ```ts  title="lib/auth-client.ts"
            import { createAuthClient } from "buttery-auth/svelte"
            export const authClient = createAuthClient({
                /** The base URL of the server (optional if you're using the same domain) */ // [!code highlight]
                baseURL: "http://localhost:3000" // [!code highlight]
            })
    ...
    <Tab value="solid" title="lib/auth-client.ts">
            ```ts title="lib/auth-client.ts"
            import { createAuthClient } from "buttery-auth/solid"
            export const authClient = createAuthClient({
                /** The base URL of the server (optional if you're using the same domain) */ // [!code highlight]
                baseURL: "http://localhost:3000" // [!code highlight]
            })
...
<Step>
### 🎉 That's it!
That's it! You're now ready to use buttery-auth in your application. Continue to [basic usage](/docs/basic-usage) to learn how to use the auth instance to sign in users.
</Step>
</Steps>
